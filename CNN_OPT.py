...
class ConvNet(nn.Module):
    def __init__(self, Conv2d_1_1=32, Conv2d_1_2=32, Conv2d_2_1=64, Conv2d_2_2=64):
        ...
        self.features = nn.Sequential(
            ...
            nn.Conv2d(in_channels=1, out_channels=Conv2d_1_1, kernel_size=3, stride=1, padding=1),

            # 批量归一化，跟上一层的out_channels大小相等，以下的通道规律也是必须要对应好的
            nn.BatchNorm2d(num_features=Conv2d_1_1),

            # 激活函数，inplace=true代表直接进行运算
            nn.ReLU(inplace=True),
            nn.Conv2d(Conv2d_1_1, Conv2d_1_2, kernel_size=3, stride=1, padding=1),
            nn.BatchNorm2d(Conv2d_1_2),
            nn.ReLU(inplace=True),
            ...
            nn.Conv2d(Conv2d_1_2, Conv2d_2_1, kernel_size=3, padding=1),
            nn.BatchNorm2d(Conv2d_2_1),
            nn.ReLU(inplace=True),
            nn.Conv2d(Conv2d_2_1, Conv2d_2_2, kernel_size=3, padding=1),
            nn.BatchNorm2d(Conv2d_2_2),
            nn.ReLU(inplace=True),
...
def train(num_epochs, _model, _device, _train_loader, _optimizer, _lr_scheduler, _criterion):
    _model.train()
...
def test(_test_loader, _model, _device, _ConvModel, _criterion):
    ...
    with torch.no_grad():  # 如果不需要 backward更新梯度，那么就要禁用梯度计算，减少内存和计算资源浪费。
        for data, target in _test_loader:
            ...
            output = _ConvModel(data.reshape(-1, 1, 28, 28))
            loss += _criterion(output, target).item()  # 添加损失值
    ...
    return loss, int(correct) / len(_test_loader.dataset)
...
def objective(argsDict={}):
    ...
    :return:
    """
    if 'BATCH_SIZE' in argsDict:
        BATCH_SIZE = argsDict['BATCH_SIZE']
    else:
        BATCH_SIZE = 64
    if 'EPOCHS' in argsDict:
        EPOCHS = argsDict['EPOCHS']
    else:
        EPOCHS = 1
    if 'learning_rate' in argsDict:
        learning_rate = argsDict['learning_rate']
    else:
        learning_rate = 0.1
    if 'Conv2d_1_1' in argsDict:
        Conv2d_1_1 = argsDict['Conv2d_1_1']
    else:
        Conv2d_1_1 = 32
    if 'Conv2d_1_2' in argsDict:
        Conv2d_1_2 = argsDict['Conv2d_1_2']
    else:
        Conv2d_1_2 = 32

    if 'Conv2d_2_1' in argsDict:
        Conv2d_2_1 = argsDict['Conv2d_2_1']
    else:
        Conv2d_2_1 = 64
    if 'Conv2d_2_2' in argsDict:
        Conv2d_2_2 = argsDict['Conv2d_2_2']
    else:
        Conv2d_2_2 = 64
    ...
    ConvModel = ConvNet(Conv2d_1_1=Conv2d_1_1, Conv2d_1_2=Conv2d_1_2,
                        Conv2d_2_1=Conv2d_2_1, Conv2d_2_2=Conv2d_2_2).to(DEVICE)
    # 定义交叉熵损失函数
    ...
    for epoch in range(1, EPOCHS + 1):
        train(epoch, ConvModel, DEVICE, train_loader, optimizer, exp_lr_scheduler, criterion)
        loss_test, acc_test = test(test_loader, ConvModel, DEVICE, ConvModel, criterion)
        loss_train, acc_train = test(train_loader, ConvModel, DEVICE, ConvModel, criterion)
    return loss_test, acc_test


def single_opt(learning_rate, EPOCHS=3):
    DEVICE = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    ...
    train_loader = torch.utils.data.DataLoader(
        ...
        batch_size=64, shuffle=True)  # 指明批量大小，打乱，这是处于后续训练的需要。
    ...
    exp_lr_scheduler = lr_scheduler.StepLR(optimizer, step_size=6, gamma=0.1)
    loss_test = 100
    train(EPOCHS, ConvModel, DEVICE, train_loader, optimizer, exp_lr_scheduler, criterion)
    loss_test, acc_test = test(test_loader, ConvModel, DEVICE, ConvModel, criterion)
    loss_train, acc_train = test(train_loader, ConvModel, DEVICE, ConvModel, criterion)
    return loss_test, acc_test
